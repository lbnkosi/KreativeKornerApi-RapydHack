package com.explr.explravenue.kreativeapi.cashadvance.controller
...
@RestController
@RequestMapping("/explore/api/createcashadvance")
object CashAdvanceController {

    @PostMapping(value = ["/newadvance"])
    fun createAdvance(@RequestBody accessPoint: AccessPointRequest): ResponseEntity<Any> {
        ...
        return try {
            val payout = PayoutController.createPayout(payoutRequest.apply { payout_amount = "500" })
            ...
            FirestoreController.create(accessPoint.user_type, accessPoint.email, "my_advances", firebaseRequest)
    ...
    private fun configureFirebaseRequest(accessPoint: AccessPointRequest, subscription: CreateSubscriptionResponse, payoutResponse: CreateSinglePayoutResponse): FirebaseCashAdvanceRequest {
        val request = FirebaseCashAdvanceRequest()
        ...
        request.interest_percentage = "10%"
        request.invoice_id = ""
